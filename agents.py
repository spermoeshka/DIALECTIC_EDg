...
⚠️ Fear & Greed (криптоиндекс) в контексте идёт из Alternative.me — тег (Источник: Alternative.me). ЗАПРЕЩЕНО (Источник: FRED) для Fear & Greed (FRED даёт ставку ФРС и CPI, не этот индекс).
...
3. ЗАПРЕЩЕНО использовать источники, которых нет в контексте:
   ❌ CoinDesk, ARK, JPMorgan, Reuters, FT, WSJ, Bloomberg, Seeking Alpha и т.д. — только если строка буквально есть в НОВОСТЯХ

4. ЗАПРЕЩЕНО придумывать исторические аналогии и обобщения:
   ❌ "как в 2020"
   ❌ "аналогично прошлому циклу"
   ❌ "история показывает", "история учит", "практика показывает" без факта из контекста
...
е) CoinDesk, ARK Invest, JPMorgan, Goldman, Seeking Alpha, Financial Times, WSJ, Bloomberg как «консенсус» / «аналитики говорят» — если этой формулировки НЕТ дословно в блоке НОВОСТЕЙ → ❌ ГАЛЛЮЦИНАЦИЯ [УДАЛИТЬ]
ж) «история показывает», «история учит», «как показывает практика» без конкретной даты и цифры из контекста → ❌ ГАЛЛЮЦИНАЦИЯ [УДАЛИТЬ]
...
- Любые ссылки на «консенсус» CoinDesk / ARK / JPMorgan / Goldman / Seeking Alpha / FT / WSJ / Bloomberg без дословной цитаты из блока НОВОСТЕЙ в контексте
- Фразы «история показывает», «история учит», «практика показывает» без факта из данных
