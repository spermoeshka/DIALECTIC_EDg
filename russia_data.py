...
async def fetch_rosstat_inflation() -> str:
    ...
    try:
        ...
        insecure = os.getenv("RUSSIA_ROSSTAT_INSECURE_SSL", "").strip().lower() in (
            "1", "true", "yes",
        )
        connector = (
            aiohttp.TCPConnector(ssl=False, family=socket.AF_INET)
            if insecure
            else _http_connector()
        )
        if insecure:
            logger.warning(
                "RUSSIA_ROSSTAT_INSECURE_SSL: SSL-проверка для Росстата отключена (только этот запрос)"
            )
